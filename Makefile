...
COVERAGE_PROFILE ?= /tmp/clickhousex-coverage.out
COVERAGE_SUMMARY ?= /tmp/clickhousex-coverage.txt
...
test-coverage:
	GOWORK=off go test -count=1 ./... -covermode=atomic -coverpkg=./... -coverprofile=$(COVERAGE_PROFILE)
	GOWORK=off go tool cover -func=$(COVERAGE_PROFILE) | tee $(COVERAGE_SUMMARY)
	@awk '/^total:/ { if ($$3 != "100.0%") { print "coverage " $$3 " is below required 100.0%"; exit 1 } found=1 } END { if (!found) { print "coverage total not found"; exit 1 } }' $(COVERAGE_SUMMARY)
